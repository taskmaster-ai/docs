...
<Tip>
You can start with a blank template or you can start with a PRD on an existing project.
</Tip>
...
You can co-write your PRD with a reasoning model (like Claude or GPT-4) using the following workflow:

1. **Chat about requirements** — explain what you want to build.
2. **Show an example PRD** — paste in the example PRD template found in .taskmaster/templates/example_prd.txt so the model understands the expected format.
...
PRD files are just plain text with Markdown-style formatting.
